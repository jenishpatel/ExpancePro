...
/**
 * ADD THIS: Finds a transaction by its ID.
 * @param {number} id - The ID of the transaction to find.
 * @returns {object|undefined} The transaction object or undefined if not found.
 */
...
/**
 * ADD THIS: Updates an existing transaction.
 * @param {number} id - The ID of the transaction to update.
 * @param {object} updatedTransactionData - An object with the new data.
 */
export function updateTransactionById(id, updatedTransactionData) {
    const transactionIndex = transactions.findIndex(t => t.id === id);
    if (transactionIndex > -1) {
        // Merge existing transaction with updated data
        transactions[transactionIndex] = { ...transactions[transactionIndex], ...updatedTransactionData };
        updateLocalStorage();
        refreshUI();
    } else {
